...
public class ServerController : Controller
{
    ...
    [HttpGet("/api/{table}")]
    public IActionResult Api(string table)
    {
        if (table.Equals("server", StringComparison.OrdinalIgnoreCase))
        {
            var servers = new[]
            {
                new { name = "WEB-PROD-05", hostname = "web-prod-01.corp.local", ip_address = "10.0.1.10", os = "Ubuntu 24.04 LTS", cpu_cores = 8, ram_gb = 32, environment = "Production", status = "Active" },
